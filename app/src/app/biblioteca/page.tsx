        ...
        <div className="grid gap-6">
          {MATERIALS.map((material) => {
            ...
            return (
              <div 
                key={material.id}
                className="bg-zinc-950 border border-zinc-900 p-6 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative overflow-hidden group hover:border-brand-green/30 transition-all duration-300"
              >
                ...
                <div className="flex items-start gap-4 flex-1">
                  {/* Ícone por Tipo */}
                  <div className={`w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center text-xl bg-zinc-900 border border-zinc-850 ${
                  ...
                  <div className="space-y-1.5 flex-1">
                    ...
                    <div className="flex gap-4 text-[10px] font-mono text-zinc-500">
                      <span>TAMANHO: <span className="text-white font-bold">{material.size}</span></span>
                      <span>FORMATO: <span className="text-white font-bold uppercase">{material.type}</span></span>
                      <span>DOWNLOADS: <span className="text-zinc-400 font-bold">{material.downloads + (isDownloaded ? 1 : 0)}</span></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-shrink-0 w-full sm:w-auto">
                  {isLocked ? (
                    <button
                      ...
                      className="w-full bg-brand-amber hover:bg-brand-amber-dark border border-brand-amber/30 hover:border-brand-amber/60 text-brand-black font-bold text-xs px-5 py-3 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer shadow-[0_0_15px_rgba(245,127,23,0.1)]"
                    >
                      <Lock className="w-3.5 h-3.5" />
                      Desbloquear Arquivo
                    </button>
                  ) : isDownloading ? (
                    <div className="w-full sm:w-[160px] bg-zinc-900 border border-zinc-850 p-2.5 rounded-xl space-y-1.5">
                      <div className="flex justify-between text-[9px] font-mono text-brand-green">
                        <span>BAIXANDO...</span>
                    ...
                    <div className="w-full sm:w-[160px] bg-brand-green/10 border border-brand-green/30 p-2.5 rounded-xl text-center text-brand-green font-mono text-[10px] flex items-center justify-center gap-1.5 animate-scale-up">
                    ...
                    <button
                      onClick={() => handleDownload(material)}
                      className="w-full bg-zinc-900 hover:bg-brand-green border border-zinc-850 hover:border-brand-green/60 text-white hover:text-white font-bold text-xs px-5 py-3 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer"
                    >
          ...
          <div className="relative bg-zinc-950 border border-brand-amber/40 shadow-[0_0_40px_rgba(245,127,23,0.08)] max-w-lg w-full p-8 rounded-2xl space-y-6">
            ...
            {successUpgrade ? (
              <div className="text-center py-8 space-y-4 animate-scale-up">
                ...
                <p className="text-[10px] text-zinc-500 tracking-widest font-mono uppercase animate-pulse">LIBERANDO ARQUIVO DE ELITE...</p>
              </div>
            ) : (
              <>
                {/* CABEÇALHO DO MODAL */}
                <div className="space-y-2 text-center">
                  ...
                  <h3 className="text-xl font-bold text-white tracking-tight">Desbloquear Download de Elite</h3>
                ...
                <div className="space-y-3 bg-zinc-900/40 border border-zinc-850 p-4 rounded-xl">
                  <span className="text-[9px] font-mono text-brand-amber tracking-widest uppercase font-bold">MATERIAIS OPERACIONAIS PREMIUM:</span>
                ...
                <div className="flex items-baseline justify-between border-t border-zinc-900 pt-4">
                  <div>
                    ...
                    <span className="text-[10px] text-zinc-400"> pagamento único</span>
                  </div>
      ...
      <footer className="mt-16 border-t border-zinc-900 bg-zinc-950/45 py-8 relative z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4 text-center">
          <div className="flex items-center gap-3 justify-center">
            ...
            <span className="text-[10px] font-mono text-zinc-500">ECR DRONES • CENTRAL DE DOWNLOADS MVP</span>
          </div>
